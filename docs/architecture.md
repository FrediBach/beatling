...
`SequencerRouting` renders compact input/output counts and connected cable anchors inside each card's existing fixed-height Patch footer. Hover tooltips and accessible names describe the source, signal, and destination of every route; the controls open the card's patch settings. Only assigned ports are rendered, and modulation destinations have separate cable anchors. Cable visibility does not change card or grid height. `PatchCables` measures these anchors without owning routing state.
...
- `egs.patch.v3` for the latest patch;
- `egs.arrangement.v3` for variations and song structure.

All reads are defensive. `normalizePatch` and `normalizeArrangement` supply defaults, constrain values, and migrate v1 patches to silent default effect sends as well as the older variation-repeat representation into song parts. The v3 readers fall back to v2 and v1 storage keys so existing sessions migrate on their next save. Patch v3 replaces the single `modSrc` / `modDst` / `modAmt` tuple with `modulations`, an array of independently sourced and scaled routes. Each of the seven destinations is a stable route identity and occurs at most once per block. Legacy tuples migrate to one route; an explicitly empty v3 array stays empty. Normalization rejects invalid/self sources and duplicate destinations, clamps depths, and strips legacy fields. Arrangement v3 carries these patches, with unchanged song-part semantics. Storage access remains wrapped in `try/catch` because privacy settings and quota failures must degrade to an in-memory session.
...
`src/lib/modulation.ts` owns route normalization, target labels, and displayed target values. `effectiveBlock` resolves each route's source through a pure callback and applies all targets together. Both audio scheduling and the UI use these domain contracts. Engine visual events include the full effective values, including chance and per-hit voice modulation, so readouts follow audible time rather than the scheduler's lookahead. Stopped views show editable base values.
