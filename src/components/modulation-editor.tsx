...
export function ModulationEditor({ index, blocks, visual, voice, onChange }: ModulationEditorProps) {
  ...
  return <section className="modulation-editor" aria-label="Modulation targets">
    ...
    <p className="modulation-intro">Choose a source LFO or CV, then a setting to move. Each target has its own depth.</p>
    {block.modulations.map((route) => {
      ...
      return <fieldset className="modulation-route" key={route.destination}>
        ...
        {route.source !== "" && blocks[Number(route.source)].kind !== "quantizer" && blocks[Number(route.source)].clk.length === 0 && <p className="modulation-note">Source {padBlock(Number(route.source))} has no clock. Enable its G clock to move the LFO.</p>}
        {(block.kind === "modulator" || block.kind === "quantizer") && ["tune", "decay", "level"].includes(route.destination) && <p className="modulation-note">Assign a voice or Bernoulli gate to this block to hear this target.</p>}
    ...
    <details className="modulation-guide">
      ...
      <p>Modulators move continuously between source clocks. Random chooses a value at each filled step and holds it until the next. Chance, Gate and mute control the block’s trigger/gate outputs; the LFO follows the Euclidean pattern. Blocks assigned a voice retain their stepped LFO. Targets sample the source when the receiving block advances; voice targets affect this block’s hits.</p>
    </details>
  </section>;
}
