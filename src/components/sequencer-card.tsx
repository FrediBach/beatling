...
const ROUTING_FIELDS: Array<keyof SequencerBlock> = ["clk", "rst", "mut", "gate", "modSrc", "modDst", "modAmt", "shape"];
    ...
    <article className={cn("sequencer-card", patchOpen && "is-selected", related && "is-related", visual.muted && "is-muted", visual.fire && "is-firing", changedFields.size > 0 && "has-variation-change")} data-testid={`block-${index + 1}`} data-block-index={index}>
      ...
      <div className="card-body">
        ...
        <div className="parameters">
          {ROW_PARAMS.map((parameter) => <ParameterRow key={parameter} index={index} parameter={parameter} value={block[parameter]} locked={locks[parameter]} changed={changedFields.has(parameter)} modulated={block.modDst === parameter && block.modSrc !== "" && block.modAmt !== 0} onRandomize={() => onParameterRandomize(parameter)} onLockToggle={() => onParameterLockToggle(parameter)} onChange={(value) => {
...
function ParameterRow({ index, parameter, value, locked, changed, modulated, onChange, onRandomize, onLockToggle }: { index: number; parameter: BlockParam; value: number; locked: boolean; changed: boolean; modulated: boolean; onChange: (value: number) => void; onRandomize: () => void; onLockToggle: () => void }) {
