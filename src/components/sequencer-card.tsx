  ...
  return (
    <article className={cn("sequencer-card", patchOpen && "is-selected", related && "is-related", visual.muted && "is-muted", visual.fire && "is-firing", changedFields.size > 0 && "has-variation-change")} data-testid={`block-${index + 1}`} data-routing-node data-block-index={index}>
      ...
      <div className="card-body">
        {showDial && (block.kind !== "modulator" ? <PatternDial steps={effective.steps} pulses={effective.pulses} rotation={effective.rot} position={visual.position} lfo={visual.lfo} fire={visual.fire} label={`${blockTag(block)}${block.div > 1 ? ` ÷${block.div}` : ""}`} patterns={rhythmsFor(block)} activeRhythm={visual.rhythmIndex ?? 0} /> : <ModulationScope compact block={block} visual={visual} />)}
