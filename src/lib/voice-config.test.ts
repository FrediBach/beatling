...
it("migrates v9 patch and arrangement storage with neutral voice defaults and saves v18", () => {
  ...
  expect(migrated.format).toBe("euclid-grid.v18");
  ...
  expect(arrangement.format).toBe("euclid-grid.arrangement.v18");
  ...
  expect(JSON.parse(localStorage.getItem("egs.patch.v18")!)).toEqual(migrated);
  expect(JSON.parse(localStorage.getItem("egs.arrangement.v18")!)).toEqual(arrangement);
});
