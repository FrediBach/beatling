...
const additions: Partial<Record<VoiceId, Record<string, number>>> = {
  kick: { bodyTone: 0 }, snare: { toneDecay: 130, pitchAmount: 1, pitchDecay: 30, noiseAttack: 0 }, clap: { burstDecay: 18, tailFilter: 0, tailAttack: 0 },
  rim: { balance: 50, noiseMode: 0, noiseDecay: 20 }, cow: { balance: 50, highDamping: 0 },
  ch: { lowpass: 20000, metalDecay: 0 }, oh: { lowpass: 20000, metalDecay: 0, chokeMode: 0, chokeRelease: 10 }, cym: { lowpass: 20000, metalDecay: 0, bellLevel: 0, bellFrequency: 800, bellDecay: 500, stickLevel: 0, stickFilter: 4500, stickDecay: 15 },
...
it("migrates v9 patch and arrangement storage with neutral voice defaults and saves v22", () => {
  ...
  expect(migrated.format).toBe("euclid-grid.v22");
  ...
  expect(arrangement.format).toBe("euclid-grid.arrangement.v22");
  ...
  expect(JSON.parse(localStorage.getItem("egs.patch.v22")!)).toEqual(migrated);
  expect(JSON.parse(localStorage.getItem("egs.arrangement.v22")!)).toEqual(arrangement);
});
