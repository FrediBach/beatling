...
it("migrates v9 patch and arrangement storage with neutral voice defaults and saves v26", () => {
  ...
  expect(migrated.format).toBe("euclid-grid.v26");
  ...
  expect(arrangement.format).toBe("euclid-grid.arrangement.v26");
  ...
  expect(JSON.parse(localStorage.getItem("egs.patch.v26")!)).toEqual(migrated);
  expect(JSON.parse(localStorage.getItem("egs.arrangement.v26")!)).toEqual(arrangement);
});
