...
const additions: Partial<Record<VoiceId, Record<string, number>>> = {
  ...
  ch: { lowpass: 20000 }, oh: { lowpass: 20000 }, cym: { lowpass: 20000 },
  lt: { overtoneLevel: 0 }, mt: { overtoneLevel: 0 }, ht: { overtoneLevel: 0 },
  bassline: { ampDecay: 0 }, lead: { filterDecay: 0, subLevel: 0 },
};

afterEach(() => localStorage.clear());

it("migrates v9 patch and arrangement storage with neutral voice defaults and saves v10", () => {
  ...
  expect(arrangement.format).toBe("euclid-grid.arrangement.v10");
  ...
  expect(JSON.parse(localStorage.getItem("egs.patch.v10")!)).toEqual(migrated);
  expect(JSON.parse(localStorage.getItem("egs.arrangement.v10")!)).toEqual(arrangement);
});
