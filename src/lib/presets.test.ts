...
describe("Hardgroove", () => {
  it("preserves the swung rim/cymbal groove with bass calls and second-bar low-tom answers", () => {
    const patch = createPresetPatch("hardgroove", 60);
    expect(patch).toMatchObject({ bpm: 138, rate: 4, swing: 8, vol: 60 });
    expect(hitsFor(patch, "kick", 1)).toEqual([0, 4, 8, 12]);
    expect(hitsFor(patch, "cym", 1)).toEqual([0, 2, 3, 4, 6, 7, 8, 10, 11, 12, 14, 15]);
    expect(hitsFor(patch, "cym", 4).filter((step) => step >= 48)).toEqual([48, 50, 52, 54, 56, 58, 60, 62]);
    expect(hitsFor(patch, "rim", 2)).toEqual([3, 6, 11, 14, 19, 22, 27, 30]);
    expect(hitsFor(patch, "clap", 2)).toEqual([8, 24]);
    expect(hitsFor(patch, "oh", 4)).toEqual([12, 28, 44]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([1, 5, 9, 13, 21, 29]);
    expect(hitsFor(patch, "lead", 2)).toEqual([3, 11]);
    expect(hitsFor(patch, "lt", 4)).toEqual([17, 25, 49, 57]);
    expect(hitsFor(patch, "cow", 4)).toEqual([]);
    for (const id of ["kick", "cym", "rim", "clap", "oh", "lt"] as const) expect(patch.voices[id].machine).toBe("909");
    expect(patch.effects.compressor.enabled).toBe(true);
    for (const id of ["kick", "rim", "lt", "clap"] as const) expect(patch.effects.sends[id].compressor).toBeGreaterThan(0);
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 700 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/16", lowCut: 1100 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    for (const id of ["kick", "rim", "lt", "clap", "cow"] as const) expect(patch.effects.sends[id].delay).toBe(0);
    for (const id of ["bassline", "cym", "oh"] as const) {
      expect(patch.effects.sends[id]).toEqual({ distortion: 0, delay: 0, reverb: 0, compressor: 0, karplus: 0 });
    }
  });

  it("outlines F minor while accenting the rim call and softening the last percussion pair", () => {
    const [patch, , , fill] = createPresetArrangement("hardgroove").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(patch, voice, step).vOct).name);
    expect(notesFor("bassline")).toEqual(["F2", "A♭2", "B♭2", "E♭3", "A♭2", "E♭3"]);
    expect(notesFor("lead")).toEqual(["A♭3", "C4"]);
    expect(modulationAt(patch, "rim", 6).level).toBeLessThan(modulationAt(patch, "rim", 3).level);
    expect(modulationAt(patch, "rim", 14).decay).toBeLessThan(modulationAt(patch, "rim", 11).decay);
    expect(modulationAt(patch, "cym", 3).level).toBeLessThan(modulationAt(patch, "cym", 2).level);
    for (const [voice, step] of [["rim", 28], ["lt", 29]] as const) {
      expect(modulationAt(fill, voice, step + 2).level).toBeLessThan(modulationAt(fill, voice, step).level);
      expect(modulationAt(fill, voice, step + 2).decay).toBeLessThan(modulationAt(fill, voice, step).decay);
    }
    for (const { patch: variation } of createPresetArrangement("hardgroove").variations) {
  ...
  it("adds a cowbell lift, exposes the rim/tom break and closes with an alternating percussion trade", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("hardgroove").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "rim", "clap", "lt"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([1, 5, 9, 13, 19, 23, 27, 31]);
    expect(hitsFor(lift, "lead", 2)).toEqual([3, 11, 23]);
    expect(hitsFor(lift, "cow", 4)).toEqual([55, 63]);
    expect(hitsFor(lift, "oh", 4)).toEqual([12, 28, 44, 52, 60]);
    expect(hitsFor(lift, "cym", 4)).toHaveLength(48);
    expect(lift.voices.bassline.custom.cutoff).toBeGreaterThan(groove.voices.bassline.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual(hitsFor(groove, "kick", 2));
    expect(hitsFor(breakdown, "rim", 2)).toEqual([3, 11, 19, 27]);
    expect(hitsFor(breakdown, "cym", 2)).toEqual([0, 4, 8, 12, 16, 20, 24, 28]);
    expect(hitsFor(breakdown, "clap", 2)).toEqual([]);
    expect(hitsFor(breakdown, "oh", 2)).toEqual([]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([1, 9, 21]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([3, 31]);
    expect(hitsFor(breakdown, "lt", 2)).toEqual([17, 25]);
    expect(hitsFor(fill, "kick", 2)).toEqual([0, 4, 8, 12, 16, 20, 24, 28]);
    expect(hitsFor(fill, "rim", 2)).toEqual([3, 6, 11, 14, 19, 22, 28, 30]);
    expect(hitsFor(fill, "lt", 2)).toEqual([17, 29, 31]);
    expect(hitsFor(fill, "bassline", 2)).toEqual([1, 5, 9, 13, 21]);
    expect(hitsFor(fill, "lead", 2)).toEqual([3, 11]);
    expect(hitsFor(fill, "clap", 2)).toEqual([8, 24]);
    expect(hitsFor(fill, "oh", 2)).toEqual([12]);
    expect(hitsFor(fill, "cym", 2)).toEqual([0, 2, 3, 4, 6, 7, 8, 10, 11, 12, 14, 15, 16, 18, 19, 20, 22, 23, 24, 26, 27]);
    for (const voice of ["rim", "lt", "bassline", "lead", "oh", "cym"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("Offbeat-Kick Techno", () => {
  it("preserves the displaced 909 kick and half-time clap under short two-bar synth phrases", () => {
    const patch = createPresetPatch("offbeat-kick-techno", 61);
    expect(patch).toMatchObject({ bpm: 142, rate: 4, swing: 0, vol: 61 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 4, 10, 12, 16, 20, 26, 28]);
    expect(hitsFor(patch, "ch", 1)).toEqual(Array.from({ length: 16 }, (_, step) => step));
    expect(hitsFor(patch, "oh", 4)).toEqual([6, 14, 22, 30, 38, 46, 54]);
    expect(hitsFor(patch, "clap", 2)).toEqual([8, 24]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([1, 5, 9, 13, 19, 25, 30]);
    expect(hitsFor(patch, "lead", 4)).toEqual([7, 31, 39, 63]);
    expect(hitsFor(patch, "rim", 4)).toEqual([63]);
    expect(hitsFor(patch, "lt", 4)).toEqual([]);
    for (const id of ["kick", "ch", "oh", "clap"] as const) expect(patch.voices[id].machine).toBe("909");
    expect(patch.effects.distortion).toMatchObject({ enabled: true, mode: "soft" });
    expect(patch.effects.sends.kick.distortion).toBeGreaterThan(0);
    expect(patch.effects.sends.bassline.distortion).toBeGreaterThan(0);
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/8D", lowCut: 1000 });
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 800 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    for (const id of ["kick", "bassline"] as const) {
      expect(patch.effects.sends[id].reverb).toBe(0);
    ...
    for (const id of ["ch", "oh"] as const) {
      expect(patch.effects.sends[id]).toEqual({ distortion: 0, delay: 0, reverb: 0, compressor: 0, karplus: 0 });
    }
  });

  it("plays a C-minor call and E-flat/B-flat answer with lighter hats and secondary bass notes", () => {
    const patch = createPresetPatch("offbeat-kick-techno");
    const modulationAt = (voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(voice, step).vOct).name);
    expect(notesFor("bassline")).toEqual(["C2", "E♭2", "G2", "E♭2", "E♭2", "G2", "E♭2"]);
    expect(notesFor("lead")).toEqual(["E♭3", "B♭3"]);
    expect(modulationAt("ch", 1).level).toBeLessThan(modulationAt("ch", 0).level);
    expect(modulationAt("bassline", 19).level).toBeLessThan(modulationAt("bassline", 25).level);
    expect(modulationAt("bassline", 30).decay).toBeLessThan(modulationAt("bassline", 25).decay);
    expect(modulationAt("rim", 31).level).toBeLessThan(0);
    for (const { patch: variation } of createPresetArrangement("offbeat-kick-techno").variations) {
      const kicks = new Set(hitsFor(variation, "kick", 4));
      expect(hitsFor(variation, "bassline", 4).some((step) => kicks.has(step))).toBe(false);
      expect(kicks.has(10)).toBe(true);
      expect(kicks.has(26)).toBe(true);
      expect(kicks.has(8)).toBe(false);
      expect(kicks.has(24)).toBe(false);
    }
  });

  it("densifies the lift, retains the displaced kick in the break and clears the ending for tom/rim replies", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("offbeat-kick-techno").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "ch", "clap"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([1, 3, 5, 7, 9, 11, 13, 15, 17, 21, 25, 29]);
    expect(hitsFor(lift, "lead", 2)).toEqual([7, 15, 19, 27]);
    expect(hitsFor(lift, "oh", 4).filter((step) => step >= 48)).toEqual([50, 54, 58, 62]);
    expect(hitsFor(lift, "rim", 4)).toEqual([55, 63]);
    expect(lift.voices.bassline.custom.cutoff).toBeGreaterThan(groove.voices.bassline.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 10, 16, 26]);
    expect(hitsFor(breakdown, "ch", 2)).toEqual(Array.from({ length: 16 }, (_, step) => step * 2));
    expect(hitsFor(breakdown, "oh", 2)).toEqual([]);
    expect(hitsFor(breakdown, "rim", 4)).toEqual([]);
    expect(hitsFor(breakdown, "clap", 2)).toEqual([8, 24]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([1, 9, 19]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([7]);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    expect(hitsFor(fill, "kick", 4)).toEqual(hitsFor(groove, "kick", 4));
    expect(hitsFor(fill, "bassline", 2)).toEqual([1, 5, 9, 13, 19]);
    expect(hitsFor(fill, "lead", 2)).toEqual([7]);
    expect(hitsFor(fill, "lt", 2)).toEqual([29]);
    expect(hitsFor(fill, "rim", 2)).toEqual([31]);
    expect(hitsFor(fill, "oh", 2)).toEqual([6, 14, 22]);
    expect(hitsFor(fill, "ch", 2)).toEqual(Array.from({ length: 28 }, (_, step) => step));
    for (const voice of ["bassline", "lead", "lt", "rim", "oh", "ch"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("Gabber-Adjacent", () => {
  it("preserves the rapid 909 kick and half-time snare beneath a two-bar synth response", () => {
    const patch = createPresetPatch("gabber-adjacent", 62);
    expect(patch).toMatchObject({ bpm: 165, rate: 4, swing: 0, vol: 62 });
    expect(hitsFor(patch, "kick", 2)).toEqual(Array.from({ length: 16 }, (_, step) => step * 2));
    expect(hitsFor(patch, "snare", 2)).toEqual([8, 24]);
    expect(hitsFor(patch, "cym", 4)).toEqual([0]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([1, 5, 9, 13, 19, 23, 27, 31]);
    expect(hitsFor(patch, "lead", 2)).toEqual([3, 11, 23]);
    expect(hitsFor(patch, "ch", 2)).toEqual([3, 7, 11, 15, 19, 23, 27, 31]);
    expect(hitsFor(patch, "oh", 4)).toEqual([]);
    for (const id of ["kick", "snare", "cym"] as const) expect(patch.voices[id].machine).toBe("909");
    expect(patch.effects.distortion).toMatchObject({ enabled: true, mode: "hard", trim: -5 });
    expect(patch.effects.sends.kick.distortion).toBeGreaterThan(patch.effects.sends.bassline.distortion);
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/16", lowCut: 1500 });
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 1100 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    for (const id of ["kick", "bassline"] as const) {
      expect(patch.effects.sends[id].delay).toBe(0);
      expect(patch.effects.sends[id].reverb).toBe(0);
  ...
  it("answers G bass notes with D and accents the quarter-note kick under a G–F–C hook", () => {
    const [patch, , , fill] = createPresetArrangement("gabber-adjacent").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(patch, voice, step).vOct).name);
    expect(notesFor("bassline")).toEqual(["G2", "G2", "G2", "G2", "D3", "D3", "D3", "D3"]);
    expect(notesFor("lead")).toEqual(["G3", "F4", "C4"]);
    expect(modulationAt(patch, "kick", 2).level).toBeLessThan(modulationAt(patch, "kick", 0).level);
    expect(modulationAt(patch, "kick", 2).decay).toBeLessThan(modulationAt(patch, "kick", 0).decay);
    expect(modulationAt(patch, "ch", 7).level).toBeLessThan(modulationAt(patch, "ch", 3).level);
    expect(modulationAt(fill, "kick", 31).level).toBeLessThan(modulationAt(fill, "kick", 28).level);
    expect(modulationAt(fill, "kick", 31).decay).toBeLessThan(modulationAt(fill, "kick", 28).decay);
    for (const { patch: variation } of createPresetArrangement("gabber-adjacent").variations) {
      const kicks = new Set(hitsFor(variation, "kick", 4));
      expect(hitsFor(variation, "bassline", 4).some((step) => kicks.has(step))).toBe(false);
    }
  });

  it("opens the synth lift, strips the break and exposes four final kick hits before restarting", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("gabber-adjacent").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "snare"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([1, 3, 5, 7, 9, 11, 13, 15, 19, 23, 27, 31]);
    expect(hitsFor(lift, "lead", 2)).toEqual([3, 7, 11, 15, 23, 31]);
    expect(hitsFor(lift, "ch", 2)).toEqual(Array.from({ length: 16 }, (_, step) => 1 + step * 2));
    expect(hitsFor(lift, "oh", 4)).toEqual([55, 63]);
    expect(hitsFor(lift, "cym", 4)).toEqual([0, 32]);
    expect(lift.voices.bassline.custom.cutoff).toBeGreaterThan(groove.voices.bassline.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 8, 16, 24]);
    expect(hitsFor(breakdown, "snare", 2)).toEqual([8, 24]);
    expect(hitsFor(breakdown, "cym", 2)).toEqual([]);
    expect(hitsFor(breakdown, "ch", 2)).toEqual([3, 11, 19, 27]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([1, 9, 19]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([3, 27]);
    expect(breakdown.effects.sends.kick.distortion).toBeLessThan(groove.effects.sends.kick.distortion);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    expect(hitsFor(fill, "kick", 2)).toEqual([...Array.from({ length: 14 }, (_, step) => step * 2), 28, 29, 30, 31]);
    expect(hitsFor(fill, "snare", 2)).toEqual([8, 24]);
    expect(hitsFor(fill, "bassline", 2)).toEqual([1, 5, 9, 13, 19, 23]);
    expect(hitsFor(fill, "lead", 2)).toEqual([3, 11]);
    expect(hitsFor(fill, "ch", 2)).toEqual([3, 7, 11, 15, 19, 23, 27]);
    expect(hitsFor(fill, "cym", 4)).toEqual([0, 32]);
    expect(fill.voices.kick.decay).toBeLessThan(groove.voices.kick.decay);
    for (const voice of ["kick", "snare", "bassline", "lead", "ch", "cym"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("2-Step", () => {
  it("keeps the swung broken beat and adds a two-bar bass climb with plucked replies", () => {
    const patch = createPresetPatch("two-step", 63);
    expect(patch).toMatchObject({ bpm: 135, rate: 4, swing: 28, vol: 63 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 7, 10, 16, 23, 26]);
    expect(hitsFor(patch, "snare", 4)).toEqual([4, 12, 20, 28, 36, 44, 52, 60, 63]);
    expect(hitsFor(patch, "ch", 1)).toEqual([0, 2, 3, 4, 6, 8, 10, 11, 12, 14]);
    expect(hitsFor(patch, "oh", 4)).toEqual([6, 22, 38]);
    expect(hitsFor(patch, "rim", 4)).toEqual([11, 27, 43, 63]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([2, 6, 13, 18, 24, 29]);
    expect(hitsFor(patch, "lead", 2)).toEqual([5, 21, 29]);
    expect(hitsFor(patch, "shk", 4)).toEqual([]);
    for (const id of ["kick", "snare", "ch", "oh", "rim"] as const) expect(patch.voices[id].machine).toBe("909");
    expect(patch.effects.compressor.enabled).toBe(true);
    for (const id of ["kick", "snare", "rim"] as const) expect(patch.effects.sends[id].compressor).toBeGreaterThan(0);
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 750 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/8D", lowCut: 1000 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    expect(patch.effects.sends.kick.reverb).toBe(0);
    for (const id of ["bassline", "ch", "oh", "shk"] as const) {
      expect(patch.effects.sends[id]).toEqual({ distortion: 0, delay: 0, reverb: 0, compressor: 0, karplus: 0 });
    }
  });

  it("plays a D-minor bass climb and F/B-flat answer with softer hat and snare pickups", () => {
    const [patch, , , fill] = createPresetArrangement("two-step").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(patch, voice, step).vOct).name);
    expect(notesFor("bassline")).toEqual(["D2", "E2", "G2", "G2", "B♭2", "C3"]);
    expect(notesFor("lead")).toEqual(["F4", "F4", "B♭4"]);
    expect(modulationAt(patch, "ch", 3).level).toBeLessThan(modulationAt(patch, "ch", 2).level);
    expect(modulationAt(patch, "snare", 63).level).toBeLessThan(modulationAt(patch, "snare", 60).level);
    expect(modulationAt(patch, "snare", 63).decay).toBeLessThan(modulationAt(patch, "snare", 60).decay);
    expect(modulationAt(fill, "snare", 27).level).toBeLessThan(modulationAt(fill, "snare", 28).level);
    expect(modulationAt(fill, "snare", 27).decay).toBeLessThan(modulationAt(fill, "snare", 28).decay);
    for (const { patch: variation } of createPresetArrangement("two-step").variations) {
      const kicks = new Set(hitsFor(variation, "kick", 4));
      expect(hitsFor(variation, "bassline", 4).some((step) => kicks.has(step))).toBe(false);
      expect(variation.swing).toBe(28);
    }
  });

  it("lifts the melody, exposes a rim backbeat and clears the final beat for the turnaround", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("two-step").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "snare", "ch"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([2, 6, 9, 13, 17, 21, 25, 29]);
    expect(hitsFor(lift, "lead", 2)).toEqual([5, 13, 19, 27]);
    expect(hitsFor(lift, "oh", 2)).toEqual([6, 14, 22, 30]);
    expect(hitsFor(lift, "rim", 4)).toEqual([11, 27, 43, 55, 63]);
    expect(hitsFor(lift, "shk", 2)).toEqual([1, 5, 9, 13, 17, 21, 25, 29]);
    expect(lift.voices.bassline.custom.cutoff).toBeGreaterThan(groove.voices.bassline.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 10, 16, 26]);
    expect(hitsFor(breakdown, "snare", 4)).toEqual([]);
    expect(hitsFor(breakdown, "rim", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(breakdown, "ch", 2)).toEqual([2, 6, 10, 14, 18, 22, 26, 30]);
    expect(hitsFor(breakdown, "oh", 4)).toEqual([]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([2, 13, 18]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([5, 29]);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    expect(hitsFor(fill, "kick", 4)).toEqual(hitsFor(groove, "kick", 4));
    expect(hitsFor(fill, "snare", 2)).toEqual([4, 12, 20, 27, 28]);
    expect(hitsFor(fill, "rim", 2)).toEqual([11, 23, 31]);
    expect(hitsFor(fill, "bassline", 2)).toEqual([2, 6, 13, 18]);
    expect(hitsFor(fill, "lead", 2)).toEqual([5]);
    expect(hitsFor(fill, "oh", 2)).toEqual([6, 22]);
    expect(hitsFor(fill, "ch", 2)).toEqual([0, 2, 3, 4, 6, 8, 10, 11, 12, 14, 16, 18, 19, 20, 22, 24, 26]);
    for (const voice of ["kick", "snare", "rim", "bassline", "lead", "ch", "oh"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("Speed Garage", () => {
  it("keeps the four-on-the-floor shuffle under a low bass hook and alternating 101 stabs", () => {
    const patch = createPresetPatch("speed-garage", 64);
    expect(patch).toMatchObject({ bpm: 132, rate: 4, swing: 16, vol: 64 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 4, 8, 12, 16, 20, 24, 28]);
    expect(hitsFor(patch, "snare", 2)).toEqual([4, 10, 12, 20, 26, 28]);
    expect(hitsFor(patch, "ch", 1)).toEqual([0, 2, 3, 4, 6, 7, 8, 10, 11, 12, 14, 15]);
    expect(hitsFor(patch, "oh", 4)).toEqual([2, 6, 10, 14, 18, 22, 26, 30, 34, 38, 42, 46, 50, 58]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([2, 6, 10, 14, 18, 22, 27, 31]);
    expect(hitsFor(patch, "lead", 2)).toEqual([7, 23]);
    expect(hitsFor(patch, "rim", 4)).toEqual([61]);
    expect(hitsFor(patch, "clap", 4)).toEqual([]);
    for (const id of ["kick", "snare", "ch", "oh"] as const) expect(patch.voices[id].machine).toBe("909");
    expect(patch.effects.distortion).toMatchObject({ enabled: true, mode: "soft", trim: -2 });
    expect(patch.effects.sends.bassline.distortion).toBeGreaterThan(0);
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 850 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/8", lowCut: 800 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    expect(patch.effects.sends.bassline.reverb).toBe(0);
    expect(patch.effects.sends.bassline.delay).toBe(0);
    for (const id of ["kick", "ch", "oh"] as const) {
      expect(patch.effects.sends[id]).toEqual({ distortion: 0, delay: 0, reverb: 0, compressor: 0, karplus: 0 });
    }
  });

  it("voices the F-sharp-minor hook and softens the snare ghost and late bass pickups", () => {
    const [patch, , , fill] = createPresetArrangement("speed-garage").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (patch: Patch, voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(patch, voice, step).vOct).name);
    expect(notesFor(patch, "bassline")).toEqual(["F♯1", "A1", "C♯2", "A1", "F♯1", "A1", "C♯2", "A1"]);
    expect(notesFor(patch, "lead")).toEqual(["F♯3", "C♯4"]);
    expect(notesFor(fill, "bassline").slice(-4)).toEqual(["C♯2", "C♯2", "B1", "A1"]);
    expect(modulationAt(patch, "snare", 10).level).toBeLessThan(modulationAt(patch, "snare", 12).level);
    expect(modulationAt(patch, "snare", 10).decay).toBeLessThan(modulationAt(patch, "snare", 12).decay);
    expect(modulationAt(patch, "ch", 3).level).toBeLessThan(modulationAt(patch, "ch", 2).level);
    expect(modulationAt(patch, "bassline", 27).level).toBeLessThan(modulationAt(patch, "bassline", 22).level);
    expect(modulationAt(fill, "bassline", 31).decay).toBeLessThan(modulationAt(fill, "bassline", 18).decay);
    for (const { patch: variation } of createPresetArrangement("speed-garage").variations) {
      const kicks = new Set(hitsFor(variation, "kick", 4));
      expect(hitsFor(variation, "bassline", 4).some((step) => kicks.has(step))).toBe(false);
      expect(variation.swing).toBe(16);
    }
  });

  it("adds a clap lift, drops the kick for the break and clears space for the descending bass return", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("speed-garage").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "snare", "ch"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([2, 6, 9, 11, 13, 15, 18, 22, 27, 31]);
    expect(hitsFor(lift, "lead", 2)).toEqual([3, 11, 23]);
    expect(hitsFor(lift, "clap", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(lift, "oh", 4)).toEqual(Array.from({ length: 16 }, (_, step) => 2 + step * 4));
    expect(hitsFor(lift, "rim", 4)).toEqual([53, 61]);
    expect(lift.voices.bassline.custom.cutoff).toBeGreaterThan(groove.voices.bassline.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([]);
    expect(hitsFor(breakdown, "snare", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(breakdown, "ch", 2)).toEqual([2, 6, 10, 14, 18, 22, 26, 30]);
    expect(hitsFor(breakdown, "oh", 2)).toEqual([]);
    expect(hitsFor(breakdown, "rim", 4)).toEqual([]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([2, 10, 18, 27]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([7, 31]);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    expect(breakdown.effects.sends.bassline.distortion).toBeLessThan(groove.effects.sends.bassline.distortion);
    expect(hitsFor(fill, "kick", 4)).toEqual(hitsFor(groove, "kick", 4));
    expect(hitsFor(fill, "snare", 2)).toEqual([4, 10, 12, 20, 28]);
    expect(hitsFor(fill, "bassline", 2)).toEqual([2, 6, 10, 14, 18, 25, 27, 29, 31]);
    expect(hitsFor(fill, "lead", 2)).toEqual([7]);
    expect(hitsFor(fill, "rim", 4)).toEqual([]);
    expect(hitsFor(fill, "oh", 2)).toEqual([2, 6, 10, 14, 18, 22, 26]);
    expect(hitsFor(fill, "ch", 2)).toEqual([0, 2, 3, 4, 6, 7, 8, 10, 11, 12, 14, 15, 16, 18, 19, 20, 22, 23, 24, 26, 27]);
    expect(fill.voices.bassline.custom.filterDecay).toBeLessThan(groove.voices.bassline.custom.filterDecay);
    for (const voice of ["kick", "snare", "bassline", "lead", "ch", "oh"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("Future Garage / Bass", () => {
  it("preserves the sparse swung drums beneath a dry minor bass and four-bar ambient melody", () => {
    const patch = createPresetPatch("future-garage", 61);
    expect(patch).toMatchObject({ bpm: 138, rate: 4, swing: 24, vol: 61 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 10, 16, 26]);
    expect(hitsFor(patch, "snare", 2)).toEqual([8, 24]);
    expect(hitsFor(patch, "rim", 2)).toEqual([4, 14, 20, 30]);
    expect(hitsFor(patch, "ch", 1)).toEqual([0, 3, 4, 7, 8, 11, 12, 15]);
    expect(hitsFor(patch, "oh", 4)).toEqual([12, 28, 44]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([3, 11, 21]);
    expect(hitsFor(patch, "lead", 4)).toEqual([5, 39, 55]);
    expect(hitsFor(patch, "lead", 8)).toEqual([5, 39, 55, 69, 103, 119]);
    expect(hitsFor(patch, "shk", 4)).toEqual([]);
    for (const id of ["kick", "snare", "rim", "ch", "oh"] as const) expect(patch.voices[id].machine).toBe("909");
    expect(patch.voices.lead.custom).toMatchObject({ waveform: 2, attack: 70, release: 1100 });
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "hall", lowCut: 1000 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/4", lowCut: 900 });
    for (const id of ["lead", "rim"] as const) {
      expect(patch.effects.sends[id].reverb).toBeGreaterThan(0);
      expect(patch.effects.sends[id].delay).toBeGreaterThan(0);
    }
    expect(patch.effects.sends.snare.reverb).toBeGreaterThan(0);
    for (const id of ["kick", "bassline", "ch", "oh", "shk"] as const) {
      expect(patch.effects.sends[id]).toEqual({ distortion: 0, delay: 0, reverb: 0, compressor: 0, karplus: 0 });
    }
  });

  it("plays A–C–E below a C–D–E melody while softening the late hats and rim", () => {
    const patch = createPresetPatch("future-garage");
    const modulationAt = (voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (voice: "bassline" | "lead", bars: number) => hitsFor(patch, voice, bars).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(voice, step).vOct).name);
    expect(notesFor("bassline", 2)).toEqual(["A1", "C2", "E2"]);
    expect(notesFor("lead", 4)).toEqual(["C4", "D4", "E4"]);
    expect(modulationAt("ch", 3).level).toBeLessThan(modulationAt("ch", 0).level);
    expect(modulationAt("rim", 14).level).toBeLessThan(modulationAt("rim", 4).level);
    expect(modulationAt("rim", 14).decay).toBeLessThan(modulationAt("rim", 4).decay);
    for (const { patch: variation } of createPresetArrangement("future-garage").variations) {
      const kicks = new Set(hitsFor(variation, "kick", 4));
      expect(hitsFor(variation, "bassline", 4).some((step) => kicks.has(step))).toBe(false);
      expect(variation.swing).toBe(24);
    }
  });

  it("lifts the phrase, pares the break down to kick/rim and lets the final snare trail into the return", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("future-garage").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "snare", "rim", "ch"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([3, 7, 11, 15, 19, 27]);
    expect(hitsFor(lift, "lead", 4)).toEqual([3, 19, 39, 55]);
    expect(hitsFor(lift, "oh", 4)).toEqual([12, 28, 44, 52, 60]);
    expect(hitsFor(lift, "shk", 2)).toEqual([1, 5, 9, 13, 17, 21, 25, 29]);
    expect(lift.voices.lead.custom.cutoff).toBeGreaterThan(groove.voices.lead.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 16]);
    expect(hitsFor(breakdown, "snare", 2)).toEqual([]);
    expect(hitsFor(breakdown, "rim", 2)).toEqual([4, 20]);
    expect(hitsFor(breakdown, "ch", 2)).toEqual([3, 11, 19, 27]);
    expect(hitsFor(breakdown, "oh", 2)).toEqual([]);
    ...
    expect(hitsFor(fill, "snare", 2)).toEqual([8, 24]);
    expect(hitsFor(fill, "rim", 2)).toEqual([4, 14, 20, 23]);
    expect(hitsFor(fill, "bassline", 2)).toEqual([3, 11, 19]);
    expect(hitsFor(fill, "lead", 2)).toEqual([5, 23]);
    expect(hitsFor(fill, "oh", 2)).toEqual([12]);
    expect(hitsFor(fill, "ch", 2)).toEqual([0, 3, 4, 7, 8, 11, 12, 15, 16, 19, 20, 23]);
    for (const voice of Object.keys(fill.voices) as VoiceId[]) {
      const cycle = hitsFor(fill, voice, 2);
      expect(cycle.every((step) => step <= 24)).toBe(true);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
});

describe("Slow 808 Soul", () => {
  it("keeps the laid-back 808 percussion beneath an E-flat-major bass and soft melodic replies", () => {
    const patch = createPresetPatch("slow-808-soul", 62);
    expect(patch).toMatchObject({ bpm: 94, rate: 4, swing: 12, vol: 62 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 10, 16, 26]);
    expect(hitsFor(patch, "rim", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(patch, "cow", 2)).toEqual([2, 10, 18]);
    expect(hitsFor(patch, "mt", 4)).toEqual([6, 14, 22, 30, 38, 46, 54]);
    expect(hitsFor(patch, "lt", 4)).toEqual([7, 15, 23, 31, 39, 47, 63]);
    expect(hitsFor(patch, "shk", 1)).toEqual([0, 2, 4, 6, 8, 10, 12, 14]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([3, 11, 17, 25]);
    expect(hitsFor(patch, "lead", 2)).toEqual([7, 23, 31]);
    for (const id of ["kick", "rim", "cow", "mt", "lt", "shk"] as const) expect(patch.voices[id].machine).toBe("808");
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 550 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/8", lowCut: 1000 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    for (const id of ["rim", "cow", "mt", "lt"] as const) {
      expect(patch.effects.sends[id].reverb).toBeGreaterThan(0);
      expect(patch.effects.sends[id].delay).toBe(0);
    }
    for (const id of ["kick", "bassline", "shk"] as const) {
      expect(patch.effects.sends[id]).toEqual({ distortion: 0, delay: 0, reverb: 0, compressor: 0, karplus: 0 });
    }
  });

  it("outlines E-flat major with tuned toms, a major-seventh reply and lighter percussion answers", () => {
    const [patch, , , fill] = createPresetArrangement("slow-808-soul").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(patch, voice, step).vOct).name);
    expect(notesFor("bassline")).toEqual(["E♭2", "G2", "B♭2", "G2"]);
    expect(notesFor("lead")).toEqual(["G3", "G3", "D4"]);
    for (const [voice, base, midi] of [["lt", 92, 51], ["mt", 138, 58]] as const) {
      const bodyFrequency = base * 2 ** (patch.voices[voice].tune / 12);
      expect(Math.abs(bodyFrequency - 440 * 2 ** ((midi - 69) / 12))).toBeLessThan(1);
    }
    expect(modulationAt(patch, "shk", 2).level).toBeLessThan(modulationAt(patch, "shk", 0).level);
    expect(modulationAt(patch, "cow", 10).level).toBeLessThan(modulationAt(patch, "cow", 2).level);
    expect(modulationAt(patch, "mt", 14).level).toBeLessThan(modulationAt(patch, "mt", 6).level);
    expect(modulationAt(patch, "lt", 15).decay).toBeLessThan(modulationAt(patch, "lt", 7).decay);
    for (const [voice, step] of [["mt", 26], ["lt", 27]] as const) {
      expect(modulationAt(fill, voice, step + 4).level).toBeLessThan(modulationAt(fill, voice, step).level);
      expect(modulationAt(fill, voice, step + 4).decay).toBeLessThan(modulationAt(fill, voice, step).decay);
    }
    for (const { patch: variation } of createPresetArrangement("slow-808-soul").variations) {
      const lowDrums = new Set([...hitsFor(variation, "kick", 4), ...hitsFor(variation, "mt", 4), ...hitsFor(variation, "lt", 4)]);
      expect(hitsFor(variation, "bassline", 4).some((step) => lowDrums.has(step))).toBe(false);
    }
  });

  it("opens a shaker lift, pares the break down to low-tom replies and closes with two tom pairs", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("slow-808-soul").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "rim"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([1, 5, 9, 13, 19, 27]);
    expect(hitsFor(lift, "lead", 2)).toEqual([7, 15, 19, 27]);
    expect(hitsFor(lift, "cow", 2)).toEqual([2, 10, 18, 26]);
    expect(hitsFor(lift, "mt", 4)).toEqual([6, 14, 22, 30, 38, 46, 54, 62]);
    expect(hitsFor(lift, "lt", 4)).toEqual([7, 15, 23, 31, 39, 47, 55, 63]);
    expect(hitsFor(lift, "shk", 4)).toEqual([...Array.from({ length: 48 }, (_, step) => step), ...Array.from({ length: 8 }, (_, step) => 48 + step * 2)]);
    expect(lift.voices.lead.custom.cutoff).toBeGreaterThan(groove.voices.lead.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 16]);
    expect(hitsFor(breakdown, "rim", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(breakdown, "cow", 2)).toEqual([]);
    expect(hitsFor(breakdown, "mt", 2)).toEqual([]);
    expect(hitsFor(breakdown, "lt", 2)).toEqual([15, 31]);
    expect(hitsFor(breakdown, "shk", 2)).toEqual([2, 6, 10, 14, 18, 22, 26, 30]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([3, 25]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([7, 27]);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    expect(hitsFor(fill, "kick", 2)).toEqual([0, 10, 16]);
    expect(hitsFor(fill, "rim", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(fill, "cow", 2)).toEqual([2, 10, 18]);
    expect(hitsFor(fill, "mt", 2)).toEqual([6, 14, 26, 30]);
    expect(hitsFor(fill, "lt", 2)).toEqual([7, 15, 27, 31]);
    expect(hitsFor(fill, "bassline", 2)).toEqual([3, 11, 19]);
    expect(hitsFor(fill, "lead", 2)).toEqual([7]);
    expect(hitsFor(fill, "shk", 2)).toEqual(Array.from({ length: 12 }, (_, step) => step * 2));
    for (const voice of ["kick", "rim", "cow", "mt", "lt", "shk", "bassline", "lead"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("Synth-Pop Mid-Tempo", () => {
  it("keeps the layered 808 backbeat beneath a two-bar bass and lead hook", () => {
    const patch = createPresetPatch("synth-pop-mid-tempo", 63);
    expect(patch).toMatchObject({ bpm: 116, rate: 4, swing: 0, vol: 63 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 8, 16, 24]);
    expect(hitsFor(patch, "snare", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(patch, "clap", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(patch, "ch", 1)).toEqual([0, 2, 4, 6, 8, 10, 12, 14]);
    expect(hitsFor(patch, "cym", 4)).toEqual([0]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([2, 6, 10, 14, 18, 22, 26, 30]);
    expect(hitsFor(patch, "lead", 2)).toEqual([1, 5, 9, 13, 19, 27]);
    expect(hitsFor(patch, "shk", 4)).toEqual([]);
    for (const id of ["kick", "snare", "clap", "ch", "cym"] as const) expect(patch.voices[id].machine).toBe("808");
    expect(patch.voices.clap.level).toBeLessThan(patch.voices.snare.level);
    expect(patch.effects.compressor.enabled).toBe(true);
    for (const id of ["kick", "snare", "clap"] as const) expect(patch.effects.sends[id].compressor).toBeGreaterThan(0);
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "studio", lowCut: 900 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/8D", lowCut: 1200 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    expect(patch.effects.sends.kick.reverb).toBe(0);
    expect(patch.effects.sends.kick.delay).toBe(0);
    for (const id of ["bassline", "ch", "cym", "shk"] as const) {
      expect(patch.effects.sends[id]).toEqual({ distortion: 0, delay: 0, reverb: 0, compressor: 0, karplus: 0 });
    }
  });

  it("plays a D-major hook over D/A bass with lighter eighths and soft snare pickups", () => {
    const [patch, , , fill] = createPresetArrangement("synth-pop-mid-tempo").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (patch: Patch, voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(patch, voice, step).vOct).name);
    expect(notesFor(patch, "bassline")).toEqual(["D2", "D2", "D2", "D2", "A2", "A2", "A2", "A2"]);
    expect(notesFor(patch, "lead")).toEqual(["D4", "F♯4", "A4", "F♯4", "E4", "G4"]);
    expect(notesFor(fill, "lead").at(-1)).toBe("D4");
    expect(modulationAt(patch, "ch", 2).level).toBeLessThan(modulationAt(patch, "ch", 0).level);
    for (const step of [29, 31]) {
      expect(modulationAt(fill, "snare", step).level).toBeLessThan(modulationAt(fill, "snare", 28).level);
      expect(modulationAt(fill, "snare", step).decay).toBeLessThan(modulationAt(fill, "snare", 28).decay);
    }
    for (const { patch: variation } of createPresetArrangement("synth-pop-mid-tempo").variations) {
      const kicks = new Set(hitsFor(variation, "kick", 4));
      expect(hitsFor(variation, "bassline", 4).some((step) => kicks.has(step))).toBe(false);
    }
  });

  it("opens the lift, exposes the clap break and clears the last backbeat for two snare pickups", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("synth-pop-mid-tempo").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "snare", "clap", "ch"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([2, 6, 10, 14, 17, 19, 21, 23, 25, 27, 29, 31]);
    expect(hitsFor(lift, "lead", 2)).toEqual([1, 5, 9, 13, 19, 23, 27, 31]);
    expect(hitsFor(lift, "shk", 2)).toEqual(Array.from({ length: 16 }, (_, step) => 1 + step * 2));
    expect(hitsFor(lift, "cym", 4)).toEqual([0, 32]);
    expect(lift.voices.lead.custom.cutoff).toBeGreaterThan(groove.voices.lead.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 16]);
    ...
    expect(hitsFor(breakdown, "ch", 2)).toEqual([2, 6, 10, 14, 18, 22, 26, 30]);
    expect(hitsFor(breakdown, "cym", 2)).toEqual([]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([2, 10, 22]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([1, 9, 23]);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    for (const voice of ["kick", "clap"] as const) expect(hitsFor(fill, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(fill, "snare", 2)).toEqual([4, 12, 20, 28, 29, 31]);
    expect(hitsFor(fill, "bassline", 2)).toEqual([2, 6, 10, 14, 18, 22]);
    expect(hitsFor(fill, "lead", 2)).toEqual([1, 5, 9, 13, 17]);
    expect(hitsFor(fill, "ch", 2)).toEqual(Array.from({ length: 14 }, (_, step) => step * 2));
    expect(hitsFor(fill, "cym", 4)).toEqual([0, 32]);
    for (const voice of ["kick", "snare", "clap", "ch", "cym", "bassline", "lead"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("Contemporary R&B", () => {
  it("preserves the slow swung beat beneath a descending bass and sparse four-bar lead phrase", () => {
    const patch = createPresetPatch("contemporary-rnb", 60);
    expect(patch).toMatchObject({ bpm: 72, rate: 4, swing: 20, vol: 60 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 10, 16, 26]);
    expect(hitsFor(patch, "clap", 2)).toEqual([8, 24]);
    expect(hitsFor(patch, "ch", 1)).toEqual([2, 6, 10, 13, 14]);
    expect(hitsFor(patch, "ch", 4).filter((step) => step >= 48)).toEqual([50, 54, 58, 62]);
    expect(hitsFor(patch, "rim", 4)).toEqual([14, 30, 46, 63]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([1, 9, 21, 29]);
    expect(hitsFor(patch, "lead", 4)).toEqual([5, 43, 59]);
    expect(hitsFor(patch, "lead", 8)).toEqual([5, 43, 59, 69, 107, 123]);
    expect(hitsFor(patch, "oh", 4)).toEqual([]);
    for (const id of ["kick", "clap", "ch", "rim"] as const) expect(patch.voices[id].machine).toBe("808");
    expect(patch.effects.distortion).toMatchObject({ enabled: true, mode: "soft", drive: 12 });
    expect(patch.effects.sends.bassline.distortion).toBeGreaterThan(0);
    expect(patch.effects.sends.bassline.reverb).toBe(0);
    expect(patch.effects.sends.bassline.delay).toBe(0);
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "studio", lowCut: 700 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/8D", lowCut: 900 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    for (const id of ["kick", "ch", "oh"] as const) {
      expect(patch.effects.sends[id]).toEqual({ distortion: 0, delay: 0, reverb: 0, compressor: 0, karplus: 0 });
  ...
  it("descends below the C-sharp root and softens the last lead reply, hat pickup and rim ending", () => {
    const [patch, , , fill] = createPresetArrangement("contemporary-rnb").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (patch: Patch, voice: "bassline" | "lead", bars: number) => hitsFor(patch, voice, bars).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(patch, voice, step).vOct).name);
    expect(notesFor(patch, "bassline", 2)).toEqual(["C♯2", "B1", "A1", "A♭1"]);
    expect(notesFor(patch, "lead", 4)).toEqual(["E4", "A♭4", "A♭4"]);
    expect(notesFor(fill, "bassline", 2)).toEqual(["C♯2", "A1", "A♭1"]);
    expect(modulationAt(patch, "bassline", 29).vOct).toBeLessThan(0);
    expect(modulationAt(patch, "lead", 59).level).toBeLessThan(modulationAt(patch, "lead", 43).level);
    expect(modulationAt(patch, "lead", 59).decay).toBeLessThan(modulationAt(patch, "lead", 43).decay);
    expect(modulationAt(patch, "ch", 13).level).toBeLessThan(modulationAt(patch, "ch", 14).level);
    expect(modulationAt(fill, "rim", 31).level).toBeLessThan(modulationAt(fill, "rim", 14).level);
    for (const { patch: variation } of createPresetArrangement("contemporary-rnb").variations) {
      const kicks = new Set(hitsFor(variation, "kick", 4));
      expect(hitsFor(variation, "bassline", 4).some((step) => kicks.has(step))).toBe(false);
      expect(variation.swing).toBe(20);
    }
  });

  it("lifts the melody, pares the break down to kick/rim and ends with a bass answer and soft rim pickup", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("contemporary-rnb").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "clap", "rim"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([1, 5, 9, 13, 21, 29]);
    expect(hitsFor(lift, "lead", 2)).toEqual([5, 13, 19, 27]);
    expect(hitsFor(lift, "ch", 1)).toEqual([0, 2, 4, 6, 8, 10, 12, 13, 14]);
    expect(hitsFor(lift, "ch", 4).filter((step) => step >= 48)).toEqual([48, 50, 52, 54, 56, 58, 60, 62, 63]);
    expect(hitsFor(lift, "oh", 4)).toEqual([60]);
    expect(lift.voices.lead.custom.cutoff).toBeGreaterThan(groove.voices.lead.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 16]);
    expect(hitsFor(breakdown, "clap", 2)).toEqual([]);
    expect(hitsFor(breakdown, "rim", 2)).toEqual([8, 24]);
    expect(hitsFor(breakdown, "ch", 2)).toEqual([2, 10, 18, 26]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([1, 21]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([5]);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    for (const voice of ["kick", "clap"] as const) expect(hitsFor(fill, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(fill, "rim", 2)).toEqual([14, 31]);
    expect(hitsFor(fill, "bassline", 2)).toEqual([1, 21, 29]);
    expect(hitsFor(fill, "lead", 2)).toEqual([5]);
    expect(hitsFor(fill, "ch", 2)).toEqual([2, 6, 10, 13, 14, 18, 22, 26]);
    for (const voice of ["kick", "clap", "rim", "ch", "bassline", "lead"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("Dembow", () => {
  it("preserves the kick/snare conversation beneath interlocking bass and a short 101 call", () => {
    const patch = createPresetPatch("dembow", 62);
    expect(patch).toMatchObject({ bpm: 96, rate: 4, swing: 0, vol: 62 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 3, 6, 8, 11, 14, 16, 19, 22, 24, 27, 30]);
    expect(hitsFor(patch, "snare", 2)).toEqual([4, 7, 12, 15, 20, 23, 28, 31]);
    expect(hitsFor(patch, "ch", 1)).toEqual([0, 2, 4, 6, 8, 10, 12, 14]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([1, 5, 9, 13, 18, 26]);
    expect(hitsFor(patch, "lead", 2)).toEqual([5, 11, 15]);
    expect(hitsFor(patch, "rim", 4)).toEqual([61]);
    expect(hitsFor(patch, "shk", 4)).toEqual([]);
    expect(hitsFor(patch, "lt", 4)).toEqual([]);
    for (const id of ["kick", "snare", "ch"] as const) expect(patch.voices[id].machine).toBe("808");
    expect(patch.effects.compressor.enabled).toBe(true);
    for (const id of ["kick", "snare", "lt"] as const) expect(patch.effects.sends[id].compressor).toBeGreaterThan(0);
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 700 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/8", lowCut: 1100 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    expect(patch.effects.sends.kick.reverb).toBe(0);
    for (const id of ["kick", "snare", "rim", "lt"] as const) expect(patch.effects.sends[id].delay).toBe(0);
    for (const id of ["bassline", "ch", "shk"] as const) {
      expect(patch.effects.sends[id]).toEqual({ distortion: 0, delay: 0, reverb: 0, compressor: 0, karplus: 0 });
    }
  });

  it("answers A/E bass with C–E–G while softening the secondary snare hits and hats", () => {
    const patch = createPresetPatch("dembow");
    const modulationAt = (voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(voice, step).vOct).name);
    expect(notesFor("bassline")).toEqual(["A2", "A2", "E3", "E3", "A2", "E3"]);
    expect(notesFor("lead")).toEqual(["C4", "E4", "G4"]);
    expect(modulationAt("snare", 7).level).toBeLessThan(modulationAt("snare", 4).level);
    expect(modulationAt("snare", 15).decay).toBeLessThan(modulationAt("snare", 12).decay);
    expect(modulationAt("ch", 2).level).toBeLessThan(modulationAt("ch", 0).level);
    for (const { patch: variation } of createPresetArrangement("dembow").variations) {
      const lowDrums = new Set([...hitsFor(variation, "kick", 4), ...hitsFor(variation, "lt", 4)]);
      expect(hitsFor(variation, "bassline", 4).some((step) => lowDrums.has(step))).toBe(false);
    }
  });

  it("adds the shaker answer, keeps the break syncopated and fits the tom/rim ending around the full beat", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("dembow").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "snare", "ch"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([1, 5, 9, 13, 17, 21, 25, 29]);
    expect(hitsFor(lift, "lead", 2)).toEqual([5, 11, 15, 19, 27]);
    expect(hitsFor(lift, "shk", 2)).toEqual(Array.from({ length: 16 }, (_, step) => 1 + step * 2));
    expect(hitsFor(lift, "rim", 4)).toEqual([53, 61]);
    expect(lift.voices.lead.custom.cutoff).toBeGreaterThan(groove.voices.lead.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([3, 11, 19, 27]);
    expect(hitsFor(breakdown, "snare", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(breakdown, "ch", 2)).toEqual([2, 6, 10, 14, 18, 22, 26, 30]);
    expect(hitsFor(breakdown, "rim", 4)).toEqual([]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([1, 9, 18]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([5, 19]);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    for (const voice of ["kick", "snare"] as const) expect(hitsFor(fill, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(fill, "bassline", 2)).toEqual([1, 5, 9, 13, 18]);
    expect(hitsFor(fill, "lead", 2)).toEqual([5, 11]);
    expect(hitsFor(fill, "lt", 2)).toEqual([25]);
    expect(hitsFor(fill, "rim", 2)).toEqual([29]);
    expect(hitsFor(fill, "ch", 2)).toEqual(Array.from({ length: 14 }, (_, step) => step * 2));
    for (const voice of ["kick", "snare", "ch", "bassline", "lead", "lt", "rim"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("Reggaeton", () => {
  it("preserves the displaced snare and continuous shaker under a two-bar bass and melody response", () => {
    const patch = createPresetPatch("reggaeton", 61);
    expect(patch).toMatchObject({ bpm: 92, rate: 4, swing: 0, vol: 61 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 3, 6, 8, 11, 14, 16, 19, 22, 24, 27, 30]);
    expect(hitsFor(patch, "snare", 2)).toEqual([3, 7, 11, 15, 19, 23, 27, 31]);
    expect(hitsFor(patch, "shk", 1)).toEqual(Array.from({ length: 16 }, (_, step) => step));
    expect(hitsFor(patch, "oh", 4)).toEqual([14, 30, 46]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([2, 10, 17, 21, 25, 29]);
    expect(hitsFor(patch, "lead", 2)).toEqual([5, 13, 17]);
    expect(hitsFor(patch, "clap", 4)).toEqual([]);
    for (const id of ["kick", "snare", "shk", "oh"] as const) expect(patch.voices[id].machine).toBe("808");
    expect(patch.effects.distortion).toMatchObject({ enabled: true, mode: "soft", drive: 16 });
    expect(patch.effects.sends.bassline.distortion).toBeGreaterThan(0);
    expect(patch.effects.sends.bassline.reverb).toBe(0);
    expect(patch.effects.sends.bassline.delay).toBe(0);
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 750 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/8D", lowCut: 1000 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    for (const id of ["kick", "shk", "oh"] as const) {
      expect(patch.effects.sends[id]).toEqual({ distortion: 0, delay: 0, reverb: 0, compressor: 0, karplus: 0 });
    }
  });

  it("climbs and descends through G minor while accenting the standalone snare hits and soft clap reply", () => {
    const [patch, , , fill] = createPresetArrangement("reggaeton").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(patch, voice, step).vOct).name);
    expect(notesFor("bassline")).toEqual(["G2", "B♭2", "D3", "C3", "B♭2", "A2"]);
    expect(notesFor("lead")).toEqual(["B♭3", "E♭4", "G3"]);
    expect(modulationAt(patch, "snare", 3).level).toBeLessThan(modulationAt(patch, "snare", 7).level);
    expect(modulationAt(patch, "snare", 11).decay).toBeLessThan(modulationAt(patch, "snare", 15).decay);
    expect(modulationAt(patch, "shk", 1).level).toBeLessThan(modulationAt(patch, "shk", 0).level);
    expect(modulationAt(fill, "clap", 29).level).toBeLessThan(modulationAt(fill, "clap", 26).level);
    for (const { patch: variation } of createPresetArrangement("reggaeton").variations) {
      const kicks = new Set(hitsFor(variation, "kick", 4));
      expect(hitsFor(variation, "bassline", 4).some((step) => kicks.has(step))).toBe(false);
    }
  });

  it("layers the clap lift, thins the break and places two ending claps between the original drum hits", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("reggaeton").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "snare", "shk"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([1, 5, 9, 13, 17, 21, 25, 29]);
    expect(hitsFor(lift, "lead", 2)).toEqual([5, 13, 19, 27]);
    expect(hitsFor(lift, "clap", 2)).toEqual([7, 15, 23, 31]);
    expect(hitsFor(lift, "oh", 4)).toEqual([14, 30, 46, 62]);
    expect(lift.voices.bassline.custom.cutoff).toBeGreaterThan(groove.voices.bassline.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 8, 16, 24]);
    expect(hitsFor(breakdown, "snare", 2)).toEqual([3, 11, 19, 27]);
    expect(hitsFor(breakdown, "shk", 2)).toEqual(Array.from({ length: 8 }, (_, step) => step * 4));
    expect(hitsFor(breakdown, "oh", 2)).toEqual([]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([2, 10, 17]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([5, 17]);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    expect(breakdown.effects.sends.bassline.distortion).toBeLessThan(groove.effects.sends.bassline.distortion);
    for (const voice of ["kick", "snare"] as const) expect(hitsFor(fill, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(fill, "bassline", 2)).toEqual([2, 10, 17, 21]);
    expect(hitsFor(fill, "lead", 2)).toEqual([5, 13]);
    expect(hitsFor(fill, "clap", 2)).toEqual([26, 29]);
    expect(hitsFor(fill, "oh", 2)).toEqual([14, 22]);
    expect(hitsFor(fill, "shk", 2)).toEqual(Array.from({ length: 28 }, (_, step) => step));
    const endingDrums = new Set([...hitsFor(fill, "kick", 2), ...hitsFor(fill, "snare", 2)]);
    expect(hitsFor(fill, "clap", 2).some((step) => endingDrums.has(step))).toBe(false);
    for (const voice of ["kick", "snare", "shk", "oh", "clap", "bassline", "lead"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("Dancehall", () => {
  it("preserves the rim backbeat and shuffled hats beneath bass and short Mixolydian stabs", () => {
    const patch = createPresetPatch("dancehall", 62);
    expect(patch).toMatchObject({ bpm: 100, rate: 4, swing: 8, vol: 62 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 8, 11, 16, 24, 27]);
    expect(hitsFor(patch, "rim", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(patch, "ch", 1)).toEqual([0, 2, 3, 4, 6, 8, 10, 11, 12, 14]);
    expect(hitsFor(patch, "mt", 4)).toEqual([6, 14, 22, 30, 38, 46, 54]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([2, 10, 17, 25]);
    expect(hitsFor(patch, "lead", 2)).toEqual([7, 15, 23]);
    expect(hitsFor(patch, "oh", 4)).toEqual([]);
    expect(hitsFor(patch, "lt", 4)).toEqual([]);
    for (const id of ["kick", "rim", "ch", "mt"] as const) expect(patch.voices[id].machine).toBe("808");
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 700 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/4", lowCut: 950 });
    for (const id of ["rim", "lead"] as const) expect(patch.effects.sends[id].delay).toBeGreaterThan(0);
  ...
  it("pairs D/A bass with F-sharp/C stabs and tuned tom answers with softer final hits", () => {
    const [patch, , , fill] = createPresetArrangement("dancehall").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(patch, voice, step).vOct).name);
    expect(notesFor("bassline")).toEqual(["D2", "A2", "D2", "A2"]);
    expect(notesFor("lead")).toEqual(["F♯3", "C4", "F♯3"]);
    for (const [voice, base, midi] of [["mt", 138, 50], ["lt", 92, 45]] as const) {
      const bodyFrequency = base * 2 ** (patch.voices[voice].tune / 12);
      expect(Math.abs(bodyFrequency - 440 * 2 ** ((midi - 69) / 12))).toBeLessThan(1);
    }
    expect(modulationAt(patch, "ch", 3).level).toBeLessThan(modulationAt(patch, "ch", 2).level);
    expect(modulationAt(patch, "mt", 14).level).toBeLessThan(modulationAt(patch, "mt", 6).level);
    for (const [voice, step] of [["mt", 21], ["lt", 23]] as const) {
      expect(modulationAt(fill, voice, step + 8).level).toBeLessThan(modulationAt(fill, voice, step).level);
      expect(modulationAt(fill, voice, step + 8).decay).toBeLessThan(modulationAt(fill, voice, step).decay);
    }
    for (const { patch: variation } of createPresetArrangement("dancehall").variations) {
      const lowDrums = new Set([...hitsFor(variation, "kick", 4), ...hitsFor(variation, "mt", 4), ...hitsFor(variation, "lt", 4)]);
      expect(hitsFor(variation, "bassline", 4).some((step) => lowDrums.has(step))).toBe(false);
    }
  });

  it("opens the melodic lift, pares the break down to kick/rim and ends with two root/fifth tom pairs", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("dancehall").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "rim", "ch"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "bassline", 2)).toEqual([1, 5, 9, 13, 17, 21, 25, 29]);
    expect(hitsFor(lift, "lead", 2)).toEqual([7, 15, 19, 23, 27, 31]);
    expect(hitsFor(lift, "oh", 4)).toEqual([30, 62]);
    expect(hitsFor(lift, "mt", 4)).toEqual([6, 14, 22, 30, 38, 46, 54, 62]);
    expect(lift.voices.lead.custom.cutoff).toBeGreaterThan(groove.voices.lead.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 8, 16, 24]);
    expect(hitsFor(breakdown, "rim", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(breakdown, "ch", 2)).toEqual([2, 6, 10, 14, 18, 22, 26, 30]);
    expect(hitsFor(breakdown, "mt", 4)).toEqual([]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([2, 17]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([7, 31]);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    for (const voice of ["kick", "rim"] as const) expect(hitsFor(fill, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(fill, "bassline", 2)).toEqual([2, 10, 17]);
    expect(hitsFor(fill, "lead", 2)).toEqual([7, 15]);
    expect(hitsFor(fill, "mt", 2)).toEqual([6, 14, 21, 29]);
    expect(hitsFor(fill, "lt", 2)).toEqual([23, 31]);
    expect(hitsFor(fill, "ch", 2)).toEqual([0, 2, 3, 4, 6, 8, 10, 11, 12, 14, 16, 18, 19, 20, 22]);
    for (const voice of ["kick", "rim", "ch", "mt", "lt", "bassline", "lead"] as const) {
      const cycle = hitsFor(fill, voice, 2);
      expect(hitsFor(fill, voice, 4)).toEqual([...cycle, ...cycle.map((step) => step + 32)]);
    }
  });
...
describe("Big Beat", () => {
  it("preserves the 909 break beneath a bass riff, four-bar hook and phrase-opening crash", () => {
    ...
    expect(patch).toMatchObject({ bpm: 122, rate: 4, swing: 12, vol: 62 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 6, 8, 16, 22, 24]);
    expect(hitsFor(patch, "snare", 2)).toEqual([4, 11, 12, 20, 27, 28]);
    expect(hitsFor(patch, "ch", 1)).toEqual([0, 2, 3, 4, 6, 8, 10, 11, 12, 14]);
    expect(hitsFor(patch, "cym", 8)).toEqual([0, 64]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([2, 10, 17, 25]);
    expect(hitsFor(patch, "lead", 4)).toEqual([7, 15, 39, 47, 51]);
    expect(hitsFor(patch, "clap", 4)).toEqual([]);
    for (const voice of ["kick", "snare", "ch", "cym"] as const) expect(patch.voices[voice].machine).toBe("909");
    expect(patch.effects.distortion).toMatchObject({ enabled: true, mode: "hard", trim: -3 });
    expect(patch.effects.sends.bassline.distortion).toBeGreaterThan(0);
    expect(patch.effects.compressor).toMatchObject({ enabled: true, ratio: 5 });
    expect(patch.effects.sends.kick.compressor).toBeGreaterThan(0);
    expect(patch.effects.sends.snare.compressor).toBeGreaterThan(0);
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 900 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/8D", lowCut: 1100 });
    expect(patch.effects.sends.lead.delay).toBeGreaterThan(0);
    for (const voice of ["kick", "bassline", "ch", "cym"] as const) {
      expect(patch.effects.sends[voice].reverb).toBe(0);
      expect(patch.effects.sends[voice].delay).toBe(0);
    }
  });

  it("plays C-minor-pentatonic phrases with lighter ghost snares and a softer final double", () => {
    const [patch, , , fill] = createPresetArrangement("big-beat").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    ...
    expect(notesFor("bassline", 2)).toEqual(["C2", "E♭2", "F2", "E♭2"]);
    expect(notesFor("lead", 4)).toEqual(["C3", "F3", "C3", "F3", "G3"]);
    expect(modulationAt(patch, "snare", 11).level).toBeLessThan(modulationAt(patch, "snare", 12).level);
    expect(modulationAt(patch, "snare", 11).decay).toBeLessThan(modulationAt(patch, "snare", 12).decay);
    expect(modulationAt(patch, "ch", 3).level).toBeLessThan(modulationAt(patch, "ch", 4).level);
    expect(modulationAt(fill, "snare", 26).level).toBeLessThan(modulationAt(fill, "snare", 24).level);
    expect(modulationAt(fill, "snare", 26).decay).toBeLessThan(modulationAt(fill, "snare", 24).decay);
    for (const { patch: variation } of createPresetArrangement("big-beat").variations) {
      const kicks = new Set(hitsFor(variation, "kick", 4));
      expect(hitsFor(variation, "bassline", 4).some((step) => kicks.has(step))).toBe(false);
    }
  });

  it("layers the lift, filters the break and leaves the last beat of the turnaround free of attacks", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("big-beat").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "snare", "ch"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "clap", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(lift, "cym", 4)).toEqual([0, 32]);
    expect(hitsFor(lift, "bassline", 2)).toEqual([2, 10, 19, 25, 30]);
    expect(hitsFor(lift, "lead", 2)).toEqual([7, 15, 19, 27]);
    expect(lift.voices.bassline.custom.cutoff).toBeGreaterThan(groove.voices.bassline.custom.cutoff);
    expect(lift.voices.lead.custom.cutoff).toBeGreaterThan(groove.voices.lead.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 8, 16, 24]);
    expect(hitsFor(breakdown, "snare", 2)).toEqual([4, 12, 20, 28]);
    expect(hitsFor(breakdown, "ch", 2)).toEqual([2, 6, 10, 14, 18, 22, 26, 30]);
    expect(hitsFor(breakdown, "cym", 4)).toEqual([]);
    expect(hitsFor(breakdown, "clap", 4)).toEqual([]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([2, 17]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([27]);
    expect(breakdown.effects.sends.bassline.distortion).toBeLessThan(groove.effects.sends.bassline.distortion);
    expect(breakdown.voices.lead.custom.release).toBeGreaterThan(groove.voices.lead.custom.release);
    expect(hitsFor(fill, "kick", 2)).toEqual([0, 6, 8, 16, 22]);
    expect(hitsFor(fill, "snare", 2)).toEqual([4, 11, 12, 20, 24, 26]);
    expect(hitsFor(fill, "ch", 2)).toEqual([0, 2, 3, 4, 6, 8, 10, 11, 12, 14, 16, 18, 19, 20, 22]);
    expect(hitsFor(fill, "cym", 2)).toEqual([0]);
    expect(hitsFor(fill, "bassline", 2)).toEqual([2, 10, 18]);
    expect(hitsFor(fill, "lead", 2)).toEqual([7, 15]);
    for (const voice of Object.keys(fill.voices) as VoiceId[]) {
      expect(hitsFor(fill, voice, 2).some((step) => step >= 28)).toBe(false);
    }
    for (const patch of [groove, lift, breakdown, fill]) {
      for (const voice of ["kick", "snare", "ch", "cym", "clap", "bassline", "lead"] as const) {
...
describe("Machine Breakbeat", () => {
  it("keeps the original broken drums beneath a two-bar synth phrase and a fourth-bar hat rest", () => {
    const patch = createPresetPatch("machine-breakbeat", 61);
    expect(patch).toMatchObject({ bpm: 130, rate: 4, swing: 8, vol: 61 });
    expect(hitsFor(patch, "kick", 2)).toEqual([0, 6, 11, 16, 22, 27]);
    expect(hitsFor(patch, "snare", 2)).toEqual([3, 4, 9, 12, 15, 19, 20, 25, 28, 31]);
    expect(hitsFor(patch, "ch", 2)).toEqual(Array.from({ length: 16 }, (_, step) => step * 2));
    expect(hitsFor(patch, "oh", 4)).toEqual([14, 30, 46]);
    expect(hitsFor(patch, "bassline", 2)).toEqual([2, 10, 21, 29]);
    expect(hitsFor(patch, "lead", 2)).toEqual([5, 13, 23]);
    expect(patch.voices.kick.machine).toBe("808");
    for (const voice of ["snare", "ch", "oh"] as const) expect(patch.voices[voice].machine).toBe("909");
    expect(patch.effects.distortion).toMatchObject({ enabled: true, mode: "soft" });
    expect(patch.effects.sends.bassline.distortion).toBeGreaterThan(0);
    expect(patch.effects.compressor).toMatchObject({ enabled: true, ratio: 4 });
    expect(patch.effects.sends.kick.compressor).toBeGreaterThan(0);
    expect(patch.effects.sends.snare.compressor).toBeGreaterThan(0);
    expect(patch.effects.reverb).toMatchObject({ enabled: true, space: "room", lowCut: 800 });
    expect(patch.effects.delay).toMatchObject({ enabled: true, sync: true, division: "1/8", lowCut: 1000 });
  ...
  it("descends through G minor, answers with root/fifth stabs and accents the snare backbeats", () => {
    const [patch, , , fill] = createPresetArrangement("machine-breakbeat").variations.map(({ patch }) => patch);
    const modulationAt = (patch: Patch, voice: VoiceId, step: number) => effectiveVoiceModulation(patch.voices[voice], (source) => {
      const block = patch.blocks[source];
      return euclideanLfoValue(block.shape, step, block, 0.5);
    });
    const notesFor = (voice: "bassline" | "lead") => hitsFor(patch, voice, 2).map((step) => quantizeVoiceCv(patch.voices[voice].custom, modulationAt(patch, voice, step).vOct).name);
    expect(notesFor("bassline")).toEqual(["G2", "F2", "E♭2", "D2"]);
    expect(notesFor("lead")).toEqual(["G3", "D4", "G3"]);
    for (const step of [3, 9, 15]) {
      expect(modulationAt(patch, "snare", step).level).toBeLessThan(modulationAt(patch, "snare", 4).level);
      expect(modulationAt(patch, "snare", step).decay).toBeLessThan(modulationAt(patch, "snare", 12).decay);
    }
    expect(modulationAt(patch, "ch", 2).level).toBeLessThan(modulationAt(patch, "ch", 0).level);
    expect(modulationAt(fill, "snare", 31).level).toBeLessThan(modulationAt(fill, "snare", 30).level);
    expect(modulationAt(fill, "snare", 30).level).toBeLessThan(modulationAt(fill, "snare", 28).level);
    for (const { patch: variation } of createPresetArrangement("machine-breakbeat").variations) {
      const kicks = new Set(hitsFor(variation, "kick", 4));
      expect(hitsFor(variation, "bassline", 4).some((step) => kicks.has(step))).toBe(false);
    }
  });

  it("brightens the lift, strips the break to backbeats and clears the final beat for chopped snares", () => {
    const [groove, lift, breakdown, fill] = createPresetArrangement("machine-breakbeat").variations.map(({ patch }) => patch);
    for (const voice of ["kick", "snare", "ch"] as const) expect(hitsFor(lift, voice, 4)).toEqual(hitsFor(groove, voice, 4));
    expect(hitsFor(lift, "oh", 4)).toEqual([14, 30, 46, 62]);
    expect(hitsFor(lift, "bassline", 2)).toEqual([2, 10, 18, 24, 29]);
    expect(hitsFor(lift, "lead", 2)).toEqual([5, 13, 19, 27]);
    expect(lift.voices.bassline.custom.cutoff).toBeGreaterThan(groove.voices.bassline.custom.cutoff);
    expect(lift.voices.lead.custom.cutoff).toBeGreaterThan(groove.voices.lead.custom.cutoff);
    expect(hitsFor(breakdown, "kick", 2)).toEqual([0, 8, 16, 24]);
    ...
    expect(hitsFor(breakdown, "ch", 2)).toEqual([2, 6, 10, 14, 18, 22, 26, 30]);
    expect(hitsFor(breakdown, "oh", 2)).toEqual([]);
    expect(hitsFor(breakdown, "bassline", 2)).toEqual([2, 21]);
    expect(hitsFor(breakdown, "lead", 2)).toEqual([23]);
    expect(hitsFor(fill, "kick", 2)).toEqual([0, 6, 11, 16, 24]);
    expect(hitsFor(fill, "snare", 2)).toEqual([3, 4, 9, 12, 15, 20, 25, 28, 30, 31]);
    expect(hitsFor(fill, "ch", 2)).toEqual(Array.from({ length: 14 }, (_, step) => step * 2));
    expect(hitsFor(fill, "oh", 2)).toEqual([14]);
    expect(hitsFor(fill, "bassline", 2)).toEqual([2, 10, 18]);
    expect(hitsFor(fill, "lead", 2)).toEqual([5, 13]);
    for (const patch of [groove, lift, breakdown, fill]) {
      for (const voice of ["kick", "snare", "ch", "oh", "bassline", "lead"] as const) {
        const cycle = hitsFor(patch, voice, 4);
        expect(hitsFor(patch, voice, 8)).toEqual([...cycle, ...cycle.map((step) => step + 64)]);
      }
    }
    // No two sequencers fire the same snare on the same tick of the fill.
    const snareParts = fill.blocks.filter((block) => block.voice === "snare");
    const isolatedHits = snareParts.flatMap((block) => hitsFor({ ...fill, blocks: [block] }, "snare", 2));
    expect(isolatedHits).toHaveLength(new Set(isolatedHits).size);
  });
});
...
describe.each(["electro-backbeat", "electro-funk-maracas", "stripped-808-breakbeat", "bass-tempo-standard", "double-time-bass", "half-time-bass-groove", "boom-bap", "sparse-808-ballad", "modern-808-hip-hop", "trap-standard", "triplet-trap", "drill-variant", "basic-house", "open-hat-house", "deep-house-shuffle", "jackin-house", "acid-basic", "acid-tom-fill", "hypnotic-acid", "detroit-syncopated-clap", "rolling-techno", "tom-driven-techno", "minimal-dub-techno", "rave-stomp", "hardgroove", "offbeat-kick-techno", "gabber-adjacent", "two-step", "speed-garage", "future-garage", "slow-808-soul", "synth-pop-mid-tempo", "contemporary-rnb", "dembow", "reggaeton", "dancehall", "afrobeats-amapiano", "machine-breakbeat", "big-beat"])("%s persistence", (id) => {
  it("round-trips every variation and keeps series, routes and sends independently editable", () => {
    ...
    for (const { patch } of arrangement.variations) {
      ...
      expect(restored.voices.kick.modulations).toEqual(patch.voices.kick.modulations);
      ...
      expect(restored.voices.rim.modulations).toEqual(patch.voices.rim.modulations);
